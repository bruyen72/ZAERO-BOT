...
import { isValidVideoBuffer, MAX_WA_VIDEO_BYTES, transcodeForWhatsapp } from './nsfwShared.js';
import { globalDownloadLimiter } from './system/limiter.js';
import { get as getShortCache, set as setShortCache } from './system/cache.js';
...
const MB = 1024 * 1024
const MAX_VIDEO_BYTES = toPositiveInt(process.env.BOT_MAX_VIDEO_BYTES, 20 * MB, 1 * MB, 200 * MB)
const MAX_JOB_MS = toPositiveInt(
  process.env.BOT_MAX_JOB_MS || process.env.BOT_FFMPEG_MAX_JOB_MS,
  90000,
  5000,
  300000,
)
...
const CONFIG = {
  ...
  RETRIES_PER_URL: 2,
  RETRY_DELAYS: [600],
  MAX_MEDIA_SIZE_BYTES: MAX_VIDEO_BYTES,
  MAX_OPTIMIZE_SOURCE_BYTES: MAX_VIDEO_BYTES,
  VIDEO_OPTIMIZER_TARGET_MB: 10,
  MAX_QUERIES_PER_COMMAND: 3,
  MAX_RESULTS_PER_PROVIDER: 10,
  MAX_PAGES_TO_TRY_PER_QUERY: 6,
  MAX_STATIC_URLS_TO_TRY: 8,
  MAX_CANDIDATES_PER_PAGE: 8,
  VIDEO_OPTIMIZER_CACHE_DIR: './cache/redgifs',
  DEFAULT_HEADERS: {
    'User-Agent':
...
const MAX_VIDEO_DOWNLOAD_BYTES = MAX_VIDEO_BYTES;
const QUERY_CACHE_TTL_MS = 60 * 1000;

class VideoOptimizer {
  constructor(config = {}) {
    this.cacheDir = config.cacheDir || CONFIG.VIDEO_OPTIMIZER_CACHE_DIR;
    const targetMb = Number(config.targetMb || CONFIG.VIDEO_OPTIMIZER_TARGET_MB);
    this.targetBytes = Math.max(3 * 1024 * 1024, Math.floor(targetMb * 1024 * 1024));
  ...
  cachePath(url = '') {
    return path.join(this.cacheDir, `${this.hash(url)}.mp4`);
  }

  pickTranscodeOptions(sizeBytes = 0, aggressive = false) {
    ...
    if (aggressive || sizeMb > 40) {
      return {
        ...
        maxBitrate: 450,
        scale: '640:-2',
        timeoutMs: Math.min(90000, MAX_JOB_MS),
        queueWaitTimeoutMs: 90000,
        limitSeconds: 15,
      };
    }
  ...
  async readCache(url = '') {
    await this.ready;
    const filePath = this.cachePath(url);

    try {
      const stats = await fs.stat(filePath);
      if (!stats.isFile() || stats.size <= 0 || stats.size > MAX_WA_VIDEO_BYTES) {
        return null;
      }
  ...
  async saveCache(url = '', buffer = Buffer.alloc(0)) {
    await this.ready;
  ...
  async optimize(url = '', sourceBuffer = Buffer.alloc(0), mediaType = 'video') {
    if (!Buffer.isBuffer(sourceBuffer) || sourceBuffer.length === 0) return null;
    if (!isValidVideoBuffer(sourceBuffer)) return null;
    ...
    // Sempre normaliza para MP4 compatível com WhatsApp.
    let normalized = await transcodeForWhatsapp(sourceBuffer, this.pickTranscodeOptions(sourceBuffer.length, false));

    // Tenta chegar perto da meta (~10MB) quando necessário.
    if (normalized.length > this.targetBytes) {
      normalized = await transcodeForWhatsapp(normalized, this.pickTranscodeOptions(normalized.length, true));
    }
    if (normalized.length > this.targetBytes) {
      normalized = await transcodeForWhatsapp(normalized, {
        preset: 'ultrafast',
        crf: 32,
        maxBitrate: 350,
        scale: '480:-2',
    ...
    if (normalized.length > MAX_WA_VIDEO_BYTES || !isValidVideoBuffer(normalized)) {
      return null;
    }

    const filePath = await this.saveCache(url, normalized);
    ...
    return {
      ...
      fromCache: false,
      compressed: mediaType === 'gif' || normalized.length < sourceBuffer.length,
      originalBytes: sourceBuffer.length,
    };
  }
}
...
const COMMAND_TO_SEARCH_TERMS = {
  ...
  // --- Bunda / Ass ---
  ass: 'big ass pawg twerking',
  booty: 'big ass pawg twerking',
  butt: 'big ass pawg twerking',
...
const COMMAND_TO_REDGIFS_TAGS = {
  ...
  // --- Bunda / Ass ---
  ass: 'big ass,pawg,twerking,bubble butt,thick',
  booty: 'big ass,pawg,twerking,bubble butt',
  butt: 'big ass,pawg,twerking,bubble butt',
...
async function getRedgifsToken(forceRefresh = false) {
  ...
  const response = await globalDownloadLimiter.run(() =>
    axios.get(url, {
...
async function fetchMediaFromUrl(url, retries = CONFIG.RETRIES_PER_URL, options = {}) {
  ...
  const maxBytes = Math.max(1, Number(options.maxBytes || CONFIG.MAX_MEDIA_SIZE_BYTES));

  for (let attempt = 0; attempt < retries; attempt += 1) {
    try {
      const buffer = await globalDownloadLimiter.run(() =>
...
function shouldApplyRedgifsSafetyFilter(options = {}) {
  if (typeof options.safetyFilter === 'boolean') return options.safetyFilter;
  return parseEnvBool(process.env.REDGIFS_SAFETY_FILTER, true);
}

function getRedgifsSafetyExcludedTerms(options = {}) {
  if (!shouldApplyRedgifsSafetyFilter(options)) return [];

  const includeDefaults =
    typeof options.includeDefaultSafetyTerms === 'boolean'
      ? options.includeDefaultSafetyTerms
  ...
  const optionTerms = parseCsvTerms(options.extraExcludedTerms || '');
  const merged = [
    ...(includeDefaults ? REDGIFS_DEFAULT_SAFETY_EXCLUDED_TERMS : []),
    ...envTerms,
    ...optionTerms,
  ];
  ...
  return uniqueStrings(
    merged
      .map((term) => mapRedgifsTermToCanonical(term) || normalizeRedgifsTag(term))
      .filter(Boolean),
  );
}

function sanitizeRedgifsTags(tags = [], blockedTerms = []) {
  const blockedSet = new Set(normalizeRedgifsTagList(blockedTerms));
  const normalizedTags = normalizeRedgifsTagList(tags);
  if (!blockedSet.size) return normalizedTags;
  return normalizedTags.filter((tag) => !blockedSet.has(tag));
}
...
const REDGIFS_NON_REAL_TERMS = new Set([
  'animation',
  'animated',
  'anime',
  'hentai',
...
const REDGIFS_EXPLICIT_NON_REAL_TERMS = new Set([
  'animation',
  'animated',
  'anime',
  'hentai',
...
const REDGIFS_EXPLICIT_MALE_TERMS = new Set([
  'male',
  'man',
  'men',
  'boy',
...
const REDGIFS_ASS_FOCUSED_TERMS = new Set([
  'ass',
  'big-ass',
  'pawg',
  'booty',
...
const REDGIFS_ASS_EXCLUDED_TERMS = new Set([
  'solo-male',
  'male-masturbation',
  'male-ass',
  'gay',
...
function buildRedgifsExcludedTerms(intent = {}, requiredTerms = []) {
  const normalizedRequired = normalizeRedgifsTagList(requiredTerms);
  const requiredSet = new Set(normalizedRequired);
  const queryTokens = new Set(
    uniqueStrings([
      ...splitQueryTerms(intent.cleanQuery || ''),
      ...splitQueryTerms(intent.cleanMapped || ''),
      ...normalizedRequired,
    ]),
  );

  const wantsNonReal = [...queryTokens].some((token) => REDGIFS_EXPLICIT_NON_REAL_TERMS.has(token));
  const wantsMale = [...queryTokens].some((token) => REDGIFS_EXPLICIT_MALE_TERMS.has(token));
  const isAssFocused =
    [...queryTokens].some((token) => REDGIFS_ASS_FOCUSED_TERMS.has(token)) ||
    requiredSet.has('big-ass') ||
    requiredSet.has('pawg');

  const excluded = [];
  if (!wantsNonReal) excluded.push(...REDGIFS_NON_REAL_TERMS);
  if (isAssFocused && !wantsMale) excluded.push(...REDGIFS_ASS_EXCLUDED_TERMS);

  return uniqueStrings(excluded.map((term) => normalizeRedgifsTag(term)).filter(Boolean));
}
...
function matchesExcludedRedgifsTerms(item = {}, excludedTerms = []) {
  if (!Array.isArray(excludedTerms) || excludedTerms.length === 0) return false;
...
export async function searchRedgifsNiches(niche, limit = CONFIG.MAX_RESULTS_PER_PROVIDER, options = {}) {
  ...
  const order = options.order || 'trending';
  const cooldownMs = getRedgifsNicheCooldownMs();
  const cacheKey = `redgifs:niche:${safeLower(niche)}:${perPage}:${page}:${order}`;
  ...
  const filtered = uniqueBy(rows, (item) => item.id || item.pageUrl).slice(0, limit);
  if (filtered.length > 0) {
    setShortCache(cacheKey, filtered, QUERY_CACHE_TTL_MS);
  }
  return filtered;
}
...
function looksLikeRedgifsDirectId(value = '') {
  const normalized = safeLower(value);
  if (!normalized) return false;
  if (!/^[a-z0-9]+$/.test(normalized)) return false;
  if (normalized.length < 12 || normalized.length > 64) return false;
  if (COMMAND_TO_SEARCH_TERMS[normalized]) return false;
  if (NICHE_TERM_MAP[normalized]) return false;
  return true;
}

function prioritizeRedgifsCandidates(candidates = [], preferredMediaType = 'video') {
  let list = prioritizeCandidates(candidates);
  if (preferredMediaType === 'gif') {
    list = [...list].sort((a, b) => {
      const aPreferred = a.mediaType === 'gif' ? 0 : 1;
      const bPreferred = b.mediaType === 'gif' ? 0 : 1;
      if (aPreferred !== bPreferred) return aPreferred - bPreferred;
      return 0;
    });
  }
  return list.slice(0, CONFIG.MAX_CANDIDATES_PER_PAGE);
}
...
async function fetchFromRedgifsUnique(commandName, options = {}) {
  ...
  const maxCandidateBytes = Math.max(0, Number(options.maxCandidateBytes || 0));
  const optimizeVideos = options.optimizeVideos === true;
  const optimizeMaxSourceBytes = Math.max(0, Number(options.optimizeMaxSourceBytes || 0));
  ...
  const forcedTags = sanitizeQuery(options.redgifsTags || options.tags || '');
  const commandTags = COMMAND_TO_REDGIFS_TAGS[normalizedCommand] || '';
  const intentTags = buildRedgifsTagsQuery(intent);
  const tagsQuery = forcedTags || commandTags || intentTags;
  const forcedTagTerms = normalizeRedgifsTagList(
    forcedTags
      .split(',')
      .map((item) => item.trim())
      .filter(Boolean),
  ).map((term) => mapRedgifsTermToCanonical(term) || term);
  const requiredTerms = uniqueStrings([...(intent.requiredTerms || []), ...forcedTagTerms]);
  const excludedTerms = buildRedgifsExcludedTerms(intent, requiredTerms);
  const safetyExcludedTerms = getRedgifsSafetyExcludedTerms({
    safetyFilter: options.safetyFilter,
    includeDefaultSafetyTerms: options.includeDefaultSafetyTerms,
    extraExcludedTerms: options.extraExcludedTerms || options.excludeTerms || '',
  });
  const blockedTerms = uniqueStrings([...excludedTerms, ...safetyExcludedTerms]);
  const preferredMediaType = safeLower(options.preferredMediaType || 'video');
  const directIdInput = sanitizeQuery(options.directIdHint || options.nicheOverride || commandName);
  const directId = looksLikeRedgifsDirectId(directIdInput) ? extractRedgifsId(directIdInput) : null;

  if (directId) {
    try {
      const parsedDirect = await parseRedgifsPage(directId);
      const parsedDirectId = safeLower(parsedDirect?.id || directId);
      if (parsedDirectId && !usedIdSet.has(parsedDirectId)) {
        const directTags = uniqueStrings(parsedDirect?.tags || []);
        const parsedDirectWithTags = { ...parsedDirect, tags: directTags };
        const termsOk =
          requiredTerms.length === 0 || matchesRequiredRedgifsTerms(parsedDirectWithTags, requiredTerms);
        const excludedHit =
          excludedTerms.length > 0 && matchesExcludedRedgifsTerms(parsedDirectWithTags, excludedTerms);
        const safetyExcludedHit =
          safetyExcludedTerms.length > 0 &&
          matchesExcludedRedgifsTerms(parsedDirectWithTags, safetyExcludedTerms);

        if (termsOk && !excludedHit && !safetyExcludedHit) {
          const directCandidates = prioritizeRedgifsCandidates(parsedDirect.candidates, preferredMediaType);
          for (const candidate of directCandidates) {
            const result = await tryDownloadCandidate(candidate, `${parsedDirect.source}:id`, {
              allowedMediaTypes,
              maxCandidateBytes,
              optimizeVideos,
              optimizeMaxSourceBytes,
            });
            if (!result) continue;

            usedIdSet.add(parsedDirectId);
            return {
              ...result,
              id: parsedDirectId,
              title: parsedDirect.title,
              pageUrl: parsedDirect.pageUrl,
              thumbnail: parsedDirect.thumbnail,
              duration: parsedDirect.duration,
              tags: sanitizeRedgifsTags(directTags, blockedTerms),
            };
          }
        }
      }
    } catch (error) {
      console.warn(`[MediaFetcher] ID direto "${directId}" nao retornou midia: ${error.message}`);
    }
  }
  ...
  // Prioriza busca por texto/tags (mais estavel e, na pratica, mais rapida).
  if (commandTags) {
    sources.push({
      type: 'search',
  ...
  // Niches continuam como fallback curado, mas sem bloquear o caminho principal.
  ...
  for (const src of uniqueSources) {
    for (let page = 1; page <= maxPages; page += 1) {
      ...
      const rowsWithoutExcluded =
        excludedTerms.length > 0
          ? rowsMatchingTerms.filter((item) => !matchesExcludedRedgifsTerms(item, excludedTerms))
          : rowsMatchingTerms;

      const rowsWithoutSafetyExcluded =
        safetyExcludedTerms.length > 0
          ? rowsWithoutExcluded.filter((item) => !matchesExcludedRedgifsTerms(item, safetyExcludedTerms))
          : rowsWithoutExcluded;

      let rowsToUse = rowsWithoutSafetyExcluded;
      if (!rowsToUse.length && !strictQuery) {
        const fallbackRows = rowsMatchingTerms.length > 0 ? rowsMatchingTerms : unseenRows;
        rowsToUse =
          safetyExcludedTerms.length > 0
            ? fallbackRows.filter((item) => !matchesExcludedRedgifsTerms(item, safetyExcludedTerms))
            : fallbackRows;
      }
      ...
      for (const row of shuffledRows) {
        try {
          ...
          if (excludedTerms.length > 0 && matchesExcludedRedgifsTerms(parsedWithTags, excludedTerms)) {
            continue;
          }
          if (safetyExcludedTerms.length > 0 && matchesExcludedRedgifsTerms(parsedWithTags, safetyExcludedTerms)) {
            continue;
          }
          const currentId = safeLower(parsed?.id || row.id || extractRedgifsId(row.pageUrl) || '');
          if (!currentId || usedIdSet.has(currentId)) continue;

          const candidates = prioritizeRedgifsCandidates(parsed.candidates, preferredMediaType);
          ...
          for (const candidate of candidates) {
            ...
            return {
              ...
              tags: sanitizeRedgifsTags(parsedTags, blockedTerms),
            };
          }
        } catch (error) {
...
export async function searchNsfwVideos(query, options = {}) {
  ...
  const cacheKey = `nsfw:search:${providerNames.join(',')}:${normalizedQuery.toLowerCase()}:${limit}`;
  const cached = getShortCache(cacheKey);
  if (Array.isArray(cached) && cached.length > 0) {
    return cached.slice(0, limit);
  }
  ...
  const result = uniqueBy(aggregated, (item) => item.pageUrl).slice(0, limit);
  if (result.length > 0) {
    setShortCache(cacheKey, result, QUERY_CACHE_TTL_MS);
  }
  return result;
}
...
export async function resolveNsfwVideo(pageUrl) {
  ...
  if (!providerName || !PROVIDERS[providerName]) {
    throw new Error('Fonte nao suportada. Use URL do RedGifs.');
  }
...
async function fetchFromStaticUrls(commandName, nsfwData = {}, options = {}) {
  ...
  for (const url of sample) {
    ...
    if (maxCandidateBytes > 0 && validation.size > 0 && validation.size > maxCandidateBytes) {
      continue;
    }
...
async function tryDownloadCandidate(candidate, fallbackSource, options = {}) {
  ...
  const maxCandidateBytes = Math.max(0, Number(options.maxCandidateBytes || 0));
  const optimizeVideos = options.optimizeVideos === true;
  const optimizeMaxSourceBytes = Math.min(
    MAX_VIDEO_BYTES,
    Math.max(
      CONFIG.MAX_MEDIA_SIZE_BYTES,
      Number(options.optimizeMaxSourceBytes || CONFIG.MAX_OPTIMIZE_SOURCE_BYTES),
    ),
  );
  ...
  const shouldOptimize = optimizeVideos && (mediaType === 'video' || mediaType === 'gif');

  if (shouldOptimize) {
    const cached = await videoOptimizer.readCache(candidate.url);
    if (cached) {
      return buildResultWithMediaInfo(
        {
          ...
          fromCache: true,
          compressed: true,
          originalMB: ((cached.size || 0) / 1024 / 1024).toFixed(2),
          finalMB: ((cached.size || 0) / 1024 / 1024).toFixed(2),
        },
        'video',
        'video/mp4',
      );
  ...
  const limitBytes = shouldOptimize ? optimizeMaxSourceBytes : CONFIG.MAX_MEDIA_SIZE_BYTES;
  if (validation.size > 0 && validation.size > limitBytes) {
    console.warn(
      shouldOptimize
        ? `[MediaFetcher] Candidate muito grande para otimizar: ${(validation.size / 1024 / 1024).toFixed(1)}MB > ${(limitBytes / 1024 / 1024).toFixed(0)}MB`
        : `[MediaFetcher] Candidate pulado: ${(validation.size / 1024 / 1024).toFixed(1)}MB > ${(limitBytes / 1024 / 1024).toFixed(0)}MB`,
    );
    return null;
  }
  ...
  if (maxCandidateBytes > 0 && validation.size > 0 && validation.size > maxCandidateBytes) {
    console.warn(
      `[MediaFetcher] Candidate pulado por limite dinamico: ${(validation.size / 1024 / 1024).toFixed(1)}MB > ${(maxCandidateBytes / 1024 / 1024).toFixed(1)}MB`,
    );
    return null;
  }

  if (validation.size > 0 && validation.size > MAX_VIDEO_BYTES) {
    console.log(
  ...
  if (shouldOptimize) {
    const optimized = await videoOptimizer.optimize(candidate.url, buffer, mediaType);
    if (!optimized?.buffer) return null;
    ...
    return buildResultWithMediaInfo(
      {
        ...
        file: optimized.path,
        url: candidate.url,
        source: `${fallbackSource}:optimized`,
        size: optimized.size,
        attempts: 1,
        mime: 'video/mp4',
        optimized: true,
        fromCache: optimized.fromCache,
        compressed: optimized.compressed,
        originalMB: (buffer.length / 1024 / 1024).toFixed(2),
        finalMB: (optimized.size / 1024 / 1024).toFixed(2),
      },
      'video',
      'video/mp4',
    );
...
export async function fetchNsfwMedia(commandName, nsfwData, options = {}) {
  ...
  if (source === 'redgifs' && uniqueIds) {
    realResult = await fetchFromRedgifsUnique(normalized, {
      ...
      maxCandidateBytes: options.maxCandidateBytes || 0,
      redgifsTags: options.redgifsTags || options.tags || '',
    ...
    realResult = await fetchFromRealProviders(normalized, {
      allowedMediaTypes,
      source,
      maxCandidateBytes: options.maxCandidateBytes || 0,
      optimizeVideos,
  ...
  const staticResult = await fetchFromStaticUrls(normalized, nsfwData, {
    allowedMediaTypes,
    maxCandidateBytes: options.maxCandidateBytes || 0,
  });
  if (staticResult) {
...
export async function fetchMediaSafe(url, options = {}) {
  ...
  let limitBytes = Math.max(1, Number(maxBytes || CONFIG.MAX_MEDIA_SIZE_BYTES));
  ...
  if (validateFirst) {
    ...
    if (contentType.startsWith('image/')) {
      limitBytes = Math.min(limitBytes, MAX_IMAGE_DOWNLOAD_BYTES);
    } else if (contentType.startsWith('video/')) {
      limitBytes = Math.min(limitBytes, MAX_VIDEO_DOWNLOAD_BYTES);
    }

    if (validation.size > 0 && validation.size > limitBytes) {
      console.warn(
        `${logPrefix} Midia acima do limite (${(validation.size / 1024 / 1024).toFixed(2)}MB > ${(limitBytes / 1024 / 1024).toFixed(2)}MB)`,
      );
      return null;
    }
  }
