...
const MB = 1024 * 1024
const MAX_WA_VIDEO_BYTES = 15 * MB
const MAX_VIDEO_BYTES = Math.max(MAX_WA_VIDEO_BYTES, parsePositiveInt(process.env.BOT_MAX_VIDEO_BYTES, 20 * MB))
const MAX_JOB_MS = Math.max(
  5000,
  Math.min(300000, parsePositiveInt(process.env.BOT_MAX_JOB_MS || process.env.BOT_FFMPEG_MAX_JOB_MS, 90000)),
)
const MAX_DOWNLOAD_VIDEO_BYTES = MAX_VIDEO_BYTES
const COMPRESS_THRESHOLD = Math.min(MAX_DOWNLOAD_VIDEO_BYTES, 20 * MB)
const REDGIFS_HISTORY_LIMIT = 500
const REDGIFS_URL_HASH_HISTORY_LIMIT = 500
...
function capTimeoutMs(value, fallback = MAX_JOB_MS) {
  return Math.max(5000, Math.min(MAX_JOB_MS, parsePositiveInt(value, fallback)))
}
...
function pickTranscodeOptions(inputSizeBytes = 0, custom = {}) {
  ...
  const fallback = {
    ...
    maxBitrate: 900,
    scale: null,
    timeoutMs: capTimeoutMs(MAX_JOB_MS),
    limitSeconds: 15,
    audioBitrate: '64k',
  }
  ...
  const sized =
    inputMb > 40
      ? { preset: 'ultrafast', crf: 28, maxBitrate: 500, scale: '720:-2', timeoutMs: capTimeoutMs(90000), limitSeconds: 15 }
      : inputMb > COMPRESS_THRESHOLD / 1024 / 1024
        ? { preset: 'veryfast', crf: 26, maxBitrate: 800, scale: null, timeoutMs: capTimeoutMs(90000), limitSeconds: 15 }
        : fallback
...
export async function transcodeForWhatsapp(inputBuffer, options = {}) {
  ...
  const timeoutMs = capTimeoutMs(cfg.timeoutMs, MAX_JOB_MS)
  const queueWaitTimeoutMs = capTimeoutMs(cfg.queueWaitTimeoutMs, Math.min(45000, MAX_JOB_MS))
  ...
  try {
    ...
    let outStat = null
    try {
      outStat = await fsp.stat(outPath)
    } catch {}
...
export function normalizeUrlForHash(value = '') {
...
export function hashRedgifsUrl(value = '') {
  const normalized = normalizeUrlForHash(value)
...
export function getChatRedgifsUrlHashHistory(chat = {}) {
  if (!Array.isArray(chat.nsfwRedgifsSentUrlHashes)) {
    chat.nsfwRedgifsSentUrlHashes = []
  }
  chat.nsfwRedgifsSentUrlHashes = chat.nsfwRedgifsSentUrlHashes
    .map((item) => normalizeId(item))
    .filter((item) => /^[a-f0-9]{40}$/.test(item))
  return chat.nsfwRedgifsSentUrlHashes
}

export function registerSentRedgifsUrlHash(chat = {}, value = '') {
  const normalized = /^[a-f0-9]{40}$/i.test(String(value || '').trim())
    ? String(value || '').trim().toLowerCase()
    : hashRedgifsUrl(value)
  if (!normalized) return

  const history = getChatRedgifsUrlHashHistory(chat).filter((item) => item !== normalized)
  history.push(normalized)
  if (history.length > REDGIFS_URL_HASH_HISTORY_LIMIT) {
    chat.nsfwRedgifsSentUrlHashes = history.slice(-REDGIFS_URL_HASH_HISTORY_LIMIT)
    return
  }
  chat.nsfwRedgifsSentUrlHashes = history
}
