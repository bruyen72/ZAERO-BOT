  ...
  async function startSocketIfNeeded(phone) {
    if (sockets.has(phone)) return sockets.get(phone);
        ...
        if (isReservedMainNumber(pho)) {
          ...
          throw new Error('Número reservado para o bot principal.');
        }
...
async function getStatus(phone) {
  ...
  async function requestPairingCode(rawPhone) {
    ...
    if (isReservedMainNumber(phoneDigits)) throw new Error("Número reservado para o bot principal.");
  ...
  async function startPairing(rawPhone) {
    ...
    if (isReservedMainNumber(phone)) {
      return { ok: false, connected: false, message: 'Número reservado para o bot principal.' };
    }
...
logger.post('/start-pairing', async (req, res) => {
  ...
  if (isReservedMainNumber(pho)) {
    ...
    return res.status(400).json({ message: 'Número reservado para o bot principal.' });
  }
  const now = Date.now();
