...
async function seeCommands() {
  ...
  // ✅ CHECK DE ESTABILIDADE (Solicitado pelo usuário)
  const criticalCmds = ['sticker', 's', 'beijo', 'lickass', 'menu']
  console.log(chalk.magentaBright('┌── [ CHECK DE COMANDOS CRÍTICOS ]'))
  criticalCmds.forEach(cmd => {
    const exists = comandos.has(cmd)
    const color = exists ? chalk.green : chalk.red
    console.log(chalk.magentaBright('│') + ` ${cmd.padEnd(10)}: ${color(exists ? '✅ Carregado' : '❌ NÃO ENCONTRADO')}`)
  })
