...
async function buildCommandIndex(dir = commandsFolder) {
  ...
  for (const fileOrFolder of items) {
    ...
    try {
      ...
      console.error(chalk.red(`⚠️ Erro de sintaxe em ${fileOrFolder}:\n${err.message}`))
      continue
    }
    ...
    try {
      ...
      console.error(chalk.red(`⚠️ Erro no plugin ${fileOrFolder}:`), e)
    }
  }
...
async function seeCommands() {
  ...
  // Log detalhado de comandos carregados
  const pluginCount = Object.keys(plugins).length
  const cmdCount = comandos.size
  console.log(chalk.green(`[AUDITORIA] ${cmdCount} comandos registrados com sucesso de ${pluginCount} plugins.`))

  // --- TAREFA 2: SANITY CHECK AUTOMÁTICO NO BOOT ---
  const criticalCommands = ['menu', 'sticker', 's', 'redgifs', 'lickass']
  console.log(chalk.cyan(`[CHECK] Iniciando verificação de integridade...`))
  
  criticalCommands.forEach(cmd => {
    const exists = comandos.has(cmd)
    if (exists) {
      console.log(chalk.green(`[CHECK] Comando "${cmd}": REGISTRADO ✅`))
    } else {
      console.error(chalk.red(`[ERRO CRÍTICO] Comando não registrado: "${cmd}" ❌`))
    }
  })
...
globalThis.reload = async (_ev, filename) => {
  ...
  if (!fs.existsSync(fullPath)) {
    console.log(chalk.yellow(`⚠️ Plugin removido: ${filename}`))
    await seeCommands()
    return
  }
  ...
  try {
    ...
    console.error(chalk.red(`⚠️ Erro de sintaxe em '${filename}'\n${err.message}`))
    return
  }
  ...
  try {
    ...
    console.error(chalk.red(`⚠️ Erro ao recarregar ${filename}:\n`), e)
  }
}
