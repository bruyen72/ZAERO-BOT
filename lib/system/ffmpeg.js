...
const MAX_CONCURRENCY = toInt(
  process.env.BOT_FFMPEG_CONCURRENCY || process.env.BOT_MAX_CONCURRENCY,
  1,
  1,
  2,
);
const MAX_WAITING_FFMPEG = toInt(process.env.BOT_FFMPEG_MAX_QUEUE, 8, 1, 64);
const MAX_JOB_MS = toInt(
  process.env.BOT_MAX_JOB_MS || process.env.BOT_FFMPEG_MAX_JOB_MS,
  90000,
  5000,
  300000,
);
const DEFAULT_QUEUE_WAIT_TIMEOUT_MS = toInt(
  process.env.BOT_FFMPEG_QUEUE_WAIT_TIMEOUT_MS,
  Math.min(45000, MAX_JOB_MS),
  5000,
  MAX_JOB_MS,
);
...
function processNextInQueue() {
  if (activeProcesses >= MAX_CONCURRENCY || waitingQueue.length === 0) return;
...
function executeFfmpeg(args = [], options = {}) {
  activeProcesses++;
  const timeoutMs = toInt(options.timeoutMs, MAX_JOB_MS, 5000, MAX_JOB_MS);
