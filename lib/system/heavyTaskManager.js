...
const MAX_CONCURRENCY = toPositiveInt(
  process.env.BOT_HEAVY_CONCURRENCY || process.env.BOT_MAX_CONCURRENCY,
  1,
  1,
  2,
);
const MAX_JOB_MS = toPositiveInt(
  process.env.BOT_MAX_JOB_MS || process.env.BOT_HEAVY_TIMEOUT_MS,
  90000,
  5000,
  300000,
);
const HEAVY_LIMIT = MAX_CONCURRENCY; // Controle global de tarefas pesadas
const HEAVY_TIMEOUT_MS = MAX_JOB_MS;
...
export const DARK_MSG = {
  processing: "⚡ 𝙕Æ𝙍Ø está despertando...\n⏳ Preparando...",
  queue: (pos) => `🩸 Um poder de cada vez...\n⏳ Fila: #${pos}\n🔥 Seja paciente. Primeiro eu mando, depois eu entrego.`,
...
export function getCommandTimeoutMs(cmdData, commandName = '', isHeavy = false) {
  ...
  if (Number.isFinite(explicit) && explicit > 0) {
    return isHeavy ? Math.min(explicit, MAX_JOB_MS) : explicit;
  }
  ...
  if (Number.isFinite(fromEnvByCommand) && fromEnvByCommand > 0) {
    return isHeavy ? Math.min(fromEnvByCommand, MAX_JOB_MS) : fromEnvByCommand;
  }
