...
const HEAVY_LIMIT = toPositiveInt(process.env.BOT_HEAVY_MAX_CONCURRENCY, 12);
const HEAVY_TIMEOUT_MS = toPositiveInt(process.env.BOT_HEAVY_TIMEOUT_MS, 180000, 1000);
...
function withTimeout(taskFn, timeoutMs, label = 'task') {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      if (settled) return;
      settled = true;
      reject(new Error(`Tempo limite excedido (${timeoutMs}ms) para ${label}.`));
    }, timeoutMs);
...
export async function executeCommandTask(taskFn, options = {}) {
  const {
    heavy = false,
    label = 'command',
  ...
  try {
    return await withTimeout(taskFn, timeoutMs, label);
  } finally {
    release();
  }
}
