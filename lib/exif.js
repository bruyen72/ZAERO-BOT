...
import { configureFluentFfmpeg } from './system/ffmpeg.js';

// Configura ffmpeg com prioridade: ENV (FFMPEG_PATH) -> PATH -> ffmpeg-static
configureFluentFfmpeg(ff);
  ...
  await new Promise((resolve, reject) => {
    ff(tmpFileIn)
      ...
      .addOutputOptions([
        ...
        'libwebp',
        '-vf',
        "scale='min(512,iw)':min'(512,ih)':force_original_aspect_ratio=decrease,fps=15,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000,format=rgba,split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse",
      ])
      .toFormat('webp')
      .save(tmpFileOut)
  })
