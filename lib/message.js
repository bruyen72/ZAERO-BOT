...
import { axiosWithTimeout, getBufferWithTimeout } from './fetchWithTimeout.js'
...
export async function getBuffer(url, options) {
  try {
    options = options || {}
    // Usa versão com timeout (10 segundos, 2 retries)
    const data = await getBufferWithTimeout(url, {
      headers: {
        DNT: 1,
...
export async function fetchJson(url, options) {
  try {
    options = options || {}
    // Usa versão com timeout (8 segundos, 2 retries)
...
export function formatDate(n, locale = 'pt-BR') {
  let d = new Date(n)
...
export function tanggal(numer) {
  ...
  return `${thisDay}, ${day} de ${myMonths[bulan]} de ${year}`
}
...
export function getSizeMedia(path) {
  return new Promise(async (resolve, reject) => {
    if (/http/.test(path)) {
      try {
        // Usa versão com timeout apenas para header
        const res = await axiosWithTimeout({
          method: 'head',
          url: path
        }, 5000, 1)
...
export async function smsg(client, m, store) {
    ...
    if (typeof content === 'object') {
      ...
      try {
        if (/^https?:\/\//.test(content)) {
          // Usa axios com timeout
          const data = await axiosWithTimeout({
            method: 'get',
            url: content,
  ...
  if (Buffer.isBuffer(path)) {
    ...
    throw new Error("Caminho ou buffer inválido")
  }
