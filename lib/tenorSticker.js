...
async function convertToWebpInternal(inputUrlOrBuffer, options = {}) {
  ...
  try {
    ...
    const attempts = [
      { fps: 15, scale: 512, q: 58, maxBytes: maxOutputBytes },
      { fps: 12, scale: 512, q: 48, maxBytes: Math.floor(maxOutputBytes * 0.95) },
      { fps: 10, scale: 448, q: 38, maxBytes: Math.floor(maxOutputBytes * 0.9) },
      { fps: 8, scale: 384, q: 30, maxBytes: Math.floor(maxOutputBytes * 0.85) }
    ]
    ...
    for (let i = 0; i < attempts.length; i += 1) {
      ...
      console.log(
        `[FIG] Conversao tentativa ${i + 1}/${attempts.length} fps=${preset.fps} scale=${preset.scale} q=${preset.q}`
      )
      ...
      try {
        await runFfmpeg(
          ...
          ],
          {
            ...
            queueWaitTimeoutMs: Math.max(ffmpegTimeoutMs, 30000)
          }
        )
      } catch (error) {
        lastError = error
