...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (text.toLowerCase() === 'clear') {
      ...
      config.owner = ''
      return m.reply(`❀ O proprietário do soquete foi removido.`)
    }
    ...
    if (!nuevo) return client.reply(m.chat, `✐ Você deve mencionar o novo proprietário do Socket.\n> Exemplo: *${usedPrefix + command}* @${idBot.split('@')[0]}`, m, { mentions: [idBot] })
    const [ownerActual, ownerNuevo] = [actual ? actual.split('@')[0] : null, nuevo.split('@')[0]]
    config.owner = nuevo
    return client.sendMessage(m.chat, { text: actual && actual !== nuevo ? `✿ O proprietário do sokect foi alterado de @${ownerActual} a @${ownerNuevo}!` : `❀ Se asignó a @${ownerNuevo} como nuevo propietario de *${config.namebot}*!`, mentions: [nuevo, ...(actual && actual !== nuevo ? [actual] : [])] }, { quoted: m })
  },
}