  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (groupAnnouncement === true) {
        return client.reply(m.chat, `《✧》 O grupo está fechado.`, m)
      }
      const applyAction = async () => {
        await client.groupSettingUpdate(m.chat, 'announcement')
        return client.reply(m.chat, `✿ O grupo foi fechado com sucesso.`, m)
      }
      if (timeout > 0) {
        await client.reply(m.chat, `❀ O grupo fechará ${clockString(timeout)}.`, m)
        setTimeout(async () => {
          try {
