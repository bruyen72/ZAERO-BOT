...
export default {
  ...
  run: async (client, m, args, command, text, prefix) => {
    ...
    if (daysArg < 1) return m.reply(`「✎」 O número de dias deve ser maior que 0.`)
    ...
    if (ranking.length === 0) return m.reply(`「✎」 Não há atividade registrada no último ${daysArg} dias.`)
    const page = parseInt(args[1]) || 1
    const perPage = 10
    const totalPages = Math.ceil(ranking.length / perPage)
    if (page < 1 || page > totalPages) return m.reply(`「✎」 Página inválida. Só existe ${totalPages} páginas disponíveis.`)
    ...
    let report = `❀ Principais mensagens nas últimas *${daysArg}* día${daysArg > 1 ? 's' : ''}\n\n`
    pageRanking.forEach((u, i) => {
      const name = db.users[u.jid]?.name || u.jid.split('@')[0]
      report += `*${start + i + 1}.* ${name}\n`
      report += `   » Mensagens: \`${u.totalMsgs}\`, Comandos: \`${u.totalCmds}\`\n`
    })
    if (page < totalPages) {
      report += `\n> Para ver a próxima página › *${prefix + command} ${daysArg} ${page + 1}*`
    }
