    ...
    try {
      ...
      for (const p of (anu.participants || [])) {
        ...
        const fakeContext = {
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              ...
              serverMessageId: '0',
              newsletterName: botSettings.nameid || botSettings.namebot || ''
            },
            externalAdReply: {
              title: botSettings.namebot || global.botName || 'Bot',
              body: global.dev || '',
              mediaUrl: null,
              description: null,
              previewType: 'PHOTO',
              thumbnailUrl: botSettings.icon || botSettings.banner || '',
              sourceUrl: botSettings.link || global?.links?.channel || '',
              mediaType: 1,
        ...
        if (anu.action === 'add' && chat?.welcome && (!primaryBotId || primaryBotId === botId)) {
          ...
          // Buscar GIF aleatório de anime welcome (DESABILITADO - usa foto de perfil)
          // try {
        ...
        if ((anu.action === 'remove' || anu.action === 'leave') && chat?.goodbye && (!primaryBotId || primaryBotId === botId)) {
          ...
          // Buscar GIF aleatório de anime goodbye/sad (DESABILITADO - usa foto de perfil)
          // try {
          //   const gifResponse = await fetchWithTimeout(`https://tenor.googleapis.com/v2/search?q=anime+goodbye+sad&key=AIzaSyCY8VRFGjKZ2wpAoRTQ3faV_XcwTrYL5DA&limit=30`)
          //   const gifData = await gifResponse.json()
          //   const gifs = gifData.results || []
          //   const randomGif = gifs[Math.floor(Math.random() * gifs.length)]
          //   const gifUrl = randomGif?.media_formats?.gif?.url || randomGif?.media_formats?.mp4?.url || pp
