export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (remainingTime > 0)
      return m.reply(`ꕥ Você deve esperar *${msToTime(remainingTime)}*antes de jogar novamente.`)
    const options = ['piedra', 'papel', 'tijera']
    const userChoice = args[0]?.trim().toLowerCase()
    if (!options.includes(userChoice)) return m.reply(`《✧》 Use o comando assim:\n› *${usedPrefix}pedra ppt*, *papel* ou *tesoura*`)
    ...
    if (result === 'win') {
      user.coins += reward
      await client.sendMessage(chatId, { text: `ꕥ Você ganhou.\n\n> ✿ *Sua escolha ›* ${userChoice}\n> ✿ *${botname} escolheu ›* ${botChoice}\n> ✿ *${monedas} ›* ¥${reward.toLocaleString()}` }, { quoted: m })
    } else if (result === 'lose') {
