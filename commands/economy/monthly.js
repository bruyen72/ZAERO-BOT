export default {
  ...
  run: async (client, m, args, usedPrefix) => {
    ...
    if (now < user.lastmonthly) {
      ...
      return client.sendMessage(m.chat, { text: `ꕥ Você já resgatou sua recompensa mensal.\n> Você pode resgatá-la novamente em *${wait}*` }, { quoted: m })
    }
    ...
    if (lost) msg += `\n> ☆ Você perdeu sua sequência de meses!`
    await client.sendMessage(m.chat, { text: `「❁」 Você reivindicou sua recompensa mensal de *+${coins.toLocaleString()} ${currency}* (Mes *${users.monthlyStreak}*)\n${msg}` }, { quoted: m })
  }
}
