...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (me.lastRoll && now < me.lastRoll) {
      ...
      return m.reply(`ꕥ Você deve esperar *${timeText.trim()}*para usar*${usedPrefix + 'rw'}* de novo.`)
    }    
    ...
    try {
      ...
      if (!media) {
        rollLocks.delete(userId)
        return m.reply(`ꕥ Nenhuma imagem encontrada para o personagem *${selected.name}*.`)
      }
      ...
      record.name = String(selected.name || 'Sem nome')
      ...
      const owner = typeof record?.user === 'string' && record.user.length ? (global.db.data.users?.[record.user]?.name || record.user.split('@')[0]).trim() : 'desconhecido'
      const msg = `❀ Nome » *${record.name}*
⚥ Gênero » *${selected.gender || 'Desconhecido'}*
✰ Valor » *${record.value.toLocaleString()}*
♡ Estado » *${record.user ? `Reivindicado por ${owner}` : 'Libre'}*
❖ Fonte » *${source}*`
