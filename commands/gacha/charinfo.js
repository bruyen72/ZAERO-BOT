...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (!tag) {
        return m.reply(`ꕥ O personagem *${character.name}*não possui tag válida para busca de imagens.`)
      }
      const mediaList = await buscarImagenDelirius(tag)
      const media = mediaList[Math.floor(Math.random() * mediaList.length)]
      if (!media) {
        return m.reply(`ꕥ Nenhuma imagem encontrada para *${character.name}*com etiqueta*${tag}*.`)
      }
      const source = getSeriesNameByCharacter(dbChars, character.id)
      const msg = `❀ Nome » *${character.name}*\n⚥ Gênero » *${character.gender || 'Desconhecido'}*\n❖ Fonte » *${source}*`
