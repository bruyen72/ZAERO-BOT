...
function getSeriesNameByCharacter(db, id) {
  return Object.entries(db).find(([, serie]) => Array.isArray(serie.characters) && serie.characters.some(c => String(c.id) === String(id)))?.[1]?.name || 'Desconhecido'
}
...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (!args.length) {
        return m.reply(`❀ Forneça um nome de personagem.\n> Exemplo » *${usedPrefix + command} Yuki Suou*`)
      }
      ...
      if (!character) {
        return m.reply(`ꕥPersonagem não encontrado*${nameQuery}*.`)
      }
      ...
      let ownerName = userEntry?.[0] ? (global.db.data.users[userEntry[0]]?.name?.trim() || userEntry[0].split('@')[0]) : 'Desconhecido'
      const localRec = chat.characters[character.id] || {}
      const claimedDateLine = (localRec.user && localRec.claimedAt) ? `\nⴵ Data da reivindicação » *${new Date(localRec.claimedAt).toLocaleDateString('es-VE', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}*` : ''
      const lastVoteAgo = typeof record.lastVotedAt === 'number' ? `faz *${formatElapsed(Date.now() - record.lastVotedAt)}*` : '*Nunca*'
      ...
      const msg = `❀ Nome » *${record.name}*
⚥ Gênero » *${character.gender || 'Desconhecido'}*
✰ Valor » *${record.value.toLocaleString()}*
♡ Estado » ${userEntry ? `Reivindicado por *${ownerName}*` : '*Libre*'}${claimedDateLine}
❖ Fonte » *${source}*
❏ Posição » *#${rank}*
