    ...
    try {
      ...
      if (intercambio) {
        if (userId !== intercambio.destinatario) {
          ...
          return m.reply(`ꕥ Apenas *${receiverName}*pode aceitar o pedido de troca.`)
        }
        ...
        if (!pA || !pB) {
          ...
          return m.reply(`ꕥ O personagem *${nurme}*não está mais disponível para troca.`)
        }
        ...
        const mensajeConfirmacion = `ꕥ *Troca concluída com sucesso (✿❛◡❛)*\n\n✎ *${intercambio.personaje1.name}*agora pertence a*${db.users[userId]?.name || userId.split('@')[0]}*\n✎ *${intercambio.personaje2.name}*agora pertence a*${db.users[intercambio.solicitante]?.name || intercambio.solicitante.split('@')[0]}*`
      ...
      if (regalo) {
        if (userId !== regalo.sender) {
          ...
          return m.reply(`ꕥ Apenas *${sname}*pode confirmar a transferência.`)
        }
        ...
        await client.sendMessage(chatId, { text: `「✿」 Você presenteou com sucesso todos os seus personagens para ${name}!\n\n> ❏ Personagens talentosos: ${regalo.count}\n> ⴵ Valor total: ${regalo.value.toLocaleString()}` }, { quoted: m })
