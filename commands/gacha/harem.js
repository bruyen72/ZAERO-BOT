  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (ownedIDs.length === 0) {
        const msg = userId === m.sender ? 'ꕥ Você não tem personagens reivindicados.' : `ꕥ *${name}*não possui caracteres reivindicados.`
      ...
      if (page < 1 || page > totalPages) {
        return m.reply(`❀ Página inválida. Há um total de *${totalPages}* páginas.`)
      }
      const start = (page - 1) * perPage
      const end = Math.min(start + perPage, ownedIDs.length)
      let message = `✿ Personagens reivindicados ✿\n`
      message += `⌦ Usuário: *${name}*\n`
      message += `♡ Personagens: *(${ownedIDs.length})*\n\n`
