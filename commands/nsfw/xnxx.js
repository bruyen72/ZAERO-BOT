...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    try {
      ...
      if (!query) return m.reply("《✧》Insira o título ou URL do vídeo XNXX.")
...
≡ Duração : ${dur || "Desconocida"}
≡ Qualidade: ${qual || "Desconocida"}
≡ Visualizações: ${views || "Desconocidas"}` }
        await client.sendMessage(m.chat, mensaje, { quoted: m })
        return
      }
      const res = await search(encodeURIComponent(query))
      if (!res.result?.length) return m.reply("《✧》 Nenhum resultado encontrado.")
...
async function xnxxdl(URL) {
  return new Promise((resolve, reject) => {
    fetch(URL, { method: "get" }).then((res) => res.text()).then((res) => {
        ...
        const info = $("span.metadata").text() || $(".metadados de vídeo.metadados").text()
        const scriptBlocks = []
        ...
        const viewsCandidates = [
          ...
          $(".video-metadata.metadata: contém('visualizações')").text(),
    ...
    fetch(`${baseurl}/search/${query}/${Math.floor(Math.random() * 3) + 1}`, { method: "get" }).then((res) => res.text()).then((res) => {
        ...
        $("div.mozaique").each(function (a, b) {
          $(b).find("div.thumb-under").each(function (c, d) {
            ...
            const title = titleAttr || titleText || $(d).find("span").text().trim() || "Sem título"
