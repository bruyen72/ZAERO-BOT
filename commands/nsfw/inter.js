...
function buildTooHeavyMessage(link = '') {
  if (!link) return 'Muito pesado. Tenta outro termo.'
  return [
...
function isTooHeavyError(error) {
  ...
  return text.includes('muito grande') || text.includes('acima do limite') || text.includes('too large')
}
    ...
    try {
      ...
      await withChatNsfwQueue(m.chat, async () => {
        ...
        try {
          ...
          await m.react('\u274C')
          if (isTooHeavyError(normalizeErr)) {
            return m.reply(buildTooHeavyMessage(mediaResult?.pageUrl || mediaResult?.url || ''))
          }
        ...
        try {
          ...
          try {
            if (isTooHeavyError(sendError)) {
              await m.reply('Vídeo pesado demais pra celular fraco. Vou mandar versão leve/SD.').catch(() => {})
            }
            retryBuffer = await loadAndNormalizeVideo({ ...mediaResult, buffer: null }, `${currentCommand}:retry`)
          } catch (retryErr) {
            if (isTooHeavyError(retryErr)) {
              await m.react('\u274C')
              return m.reply(buildTooHeavyMessage(mediaResult?.pageUrl || mediaResult?.url || ''))
            }
            throw retryErr
