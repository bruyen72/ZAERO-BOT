  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    try {
      ...
      if (!videos || videos.length === 0) {
        return m.reply('❌ Nenhum vídeo disponível para este comando.')
      }

      const randomVideo = videos[Math.floor(Math.random() * videos.length)];

      // ⚡ Baixa vídeo com timeout de 15 segundos
      console.log('📥 Baixando vídeo:', randomVideo)
      const response = await fetchWithTimeout(randomVideo, {}, 15000)
      const buffer = await response.buffer()
      console.log('✅ Vídeo baixado, enviando...')
      ...
      console.error('❌ Erro no comando inter:', e.message)
