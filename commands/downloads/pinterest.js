...
export default {
  ...
  run: async (client, m, args, command) => {
    ...
    if (!text) {
      return m.reply('《✧》 Insira um termo de pesquisa ou link do Pinterest.')
    }
    try {
      if (isPinterestUrl) {
        const data = await getPinterestDownload(text)
        if (!data) return m.reply('ꕥ Não foi possível obter o conteúdo.')

        // Limpar descrição removendo hashtags e símbolos decorativos
        let cleanDesc = data.description || ''
        if (cleanDesc) {
          // Remove hashtags
          cleanDesc = cleanDesc.replace(/#\S+/g, '')
          // Remove símbolos decorativos (•, 【】, ═, etc)
          cleanDesc = cleanDesc.replace(/[•═【】✨🌙©️]/g, '')
          // Remove emojis de telefone/Instagram
          cleanDesc = cleanDesc.replace(/📸|📡|Ⓗⓐⓢⓣⓐⓖⓢ|Instagram/gi, '')
          // Remove múltiplos espaços e quebras de linha
          cleanDesc = cleanDesc.replace(/\s+/g, ' ').trim()
          // Limita a 150 caracteres se muito longo
          if (cleanDesc.length > 150) cleanDesc = cleanDesc.substring(0, 150) + '...'
          // Se ficou vazio ou muito curto, não mostra
          if (cleanDesc.length < 10) cleanDesc = null
        }

        const caption = `╔═══『 📌 PINTEREST 』═══╗\n` +
          `║\n` +
          `${data.title ? `║ 📝 *Título:* ${data.title}\n` : ''}` +
          `${cleanDesc ? `║ 📄 *Descrição:* ${cleanDesc}\n` : ''}` +
          `${data.likes ? `║ ❤️ *Likes:* ${data.likes}\n` : ''}` +
          `${data.comments ? `║ 💬 *Comentários:* ${data.comments}\n` : ''}` +
          `${data.views ? `║ 👁️ *Visualizações:* ${data.views}\n` : ''}` +
          `${data.saved ? `║ 💾 *Salvos:* ${data.saved}\n` : ''}` +
        ...
        const medias = results.slice(0, 10).map(r => {
          ...
          if (cleanDesc) {
            cleanDesc = cleanDesc.replace(/#\S+/g, '').replace(/[•═【】✨🌙©️]/g, '').replace(/📸|📡|Ⓗⓐⓢⓣⓐⓖⓢ|Instagram/gi, '').replace(/\s+/g, ' ').trim()
            if (cleanDesc.length > 100) cleanDesc = cleanDesc.substring(0, 100) + '...'
            if (cleanDesc.length < 10) cleanDesc = null
          }

          let caption = `╔═══『 🔍 PINTEREST 』═══╗\n║\n`
          if (r.title) caption += `║ 📝 *Título:* ${r.title}\n`
          if (cleanDesc) caption += `║ 📄 *Descrição:* ${cleanDesc}\n`
          if (r.likes) caption += `║ ❤️ *Likes:* ${r.likes}\n`
          caption += `║\n╚═══『 ✧ ZÆRØ BOT ✧ 』═══╝`
...
async function getPinterestDownload(url) {
  ...
  for (const { endpoint, extractor } of apis) {
    try {
      const res = await fetchWithTimeout(endpoint).then(r => r.json())
