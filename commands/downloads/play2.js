...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      if (!args[0]) {
        return m.reply('《✧》Por favor mencione o nome ou URL do vídeo que deseja baixar')
      }
      ...
      try {
        ...
        if (search.all.length) {
          ...
          if (videoInfo) {
            ...
            const canal = videoInfo.author?.name || 'Desconhecido'
            const infoMessage = `➩ Baixando › *${title}*

> ❖ Canal › *${canal}*
> ⴵ Duração › *${videoInfo.timestamp || 'Desconhecido'}*
> ❀ Visualizações › *${vistas}*
> ✩ Publicado › *${videoInfo.ago || 'Desconhecido'}*
> ❒ Link › *${url}*`
      ...
      if (!video?.url) {
        return m.reply('《✧》 Não foi possível baixar o *vídeo*, tente novamente mais tarde.')
      }
...
async function getVideoFromApis(url) {
  ...
  for (const { api, endpoint, extractor } of apis) {
    try {
      ...
      const res = await fetchWithTimeout(endpoint, { signal: controller.signal }).then(r => r.json())
