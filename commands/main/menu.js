...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      console.log('📋 Comando MENU iniciado')
      // ⚡ Reação em background (não bloqueia)
      m.react('⏳').catch(() => {})
      ...
      if (!menuData) {
        ...
        // ⚡ Cache de 10 minutos (600 segundos) para melhor performance
        apiCache.set(cacheKey, menuData, 600)
      }
...
${menu}`
      ...
      console.log('📝 Menu length:', headerText.length)

      // ⚡ Prepara mensagem sem await múltiplo
      let sendPromise;

      if (typeof banner === 'string' && (banner.includes('.mp4') || banner.includes('.webm'))) {
        console.log('📹 Enviando menu com vídeo...')
        sendPromise = client.sendMessage(m.chat, {
          video: { url: banner },
          gifPlayback: true,
          caption: headerText,
          ...mentionContext
        ...
        console.log('🖼️ Enviando menu com imagem...')
        sendPromise = client.sendMessage(m.chat, {
          image: { url: logoPath },
          caption: headerText,
          ...mentionContext
        ...
        console.log('💬 Enviando menu como texto...')
        sendPromise = client.sendMessage(m.chat, {
          text: headerText,
          ...mentionContext
        }, { quoted: m })
      ...
      // ⚡ Reação final em paralelo (não bloqueia)
      sendPromise.then(() => {
      ...
      await sendPromise
      console.log('🎯 Menu promise concluída')
    } catch (e) {
      await m.react('❌').catch(() => {})
