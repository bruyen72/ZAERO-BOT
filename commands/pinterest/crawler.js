...
async function fetchWithTimeout(url, init = {}) {
  ...
  try {
    return await globalDownloadLimiter.run(() =>
...
async function searchPinterestPinsInternal(queryOrUrl, options = {}) {
  ...
  const allowTermCache = normalized.kind === 'query' && options.requireAuth !== true
  const cacheKey = allowTermCache
    ? `pin-search:${normalized.query.toLowerCase()}:${maxResults}:${maxPages}`
    : ''
  const cached = cacheKey ? getShortCache(cacheKey) : null
  if (cached && Array.isArray(cached.pins)) {
    return {
      searchUrl: String(cached.searchUrl || normalized.searchUrl || ''),
      pins: cached.pins,
      cookieJar: null,
    }
  }
