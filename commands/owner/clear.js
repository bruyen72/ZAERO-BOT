export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!chat?.users || typeof chat.users !== 'object')
      return m.reply('ꕥ Nenhum dado de grupo encontrado.')
    ...
    for (const jid of Object.keys(chat.users)) {
      ...
      eliminados++
      const tiempo = u.lastCmd > 0 ? formatTime(delta) : 'sem registro prévio'
      userList.push(`@${jid.split('@')[0]} *[${u.characters.length}]* - ${tiempo}`)
      mentions.push(jid)
    }
    if (!eliminados)
      return m.reply('ꕥ Nenhum usuário inativo foi encontrado. \n> ⴵ Prazo: 40 dias')
    const report = ['❀ Os dados do usuário inativo foram excluídos.', `> ♡ Reivindicações excluídas: ${waifus}`, `> ⛁ Moedas removidas: ${dinero.toLocaleString()}`, `> ❖ Usuários inativos: ${eliminados}`, `> ⴵ Prazo: 40 dias`, `> ❏ O comando foi executado em ${getDuration(start)}ms`, '', ...userList,].join('\n')
