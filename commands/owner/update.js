  ...
  async function readCommands(folder) {
    ...
    for (const file of files) {
      ...
      if (fs.lstatSync(fullPath).isDirectory()) {
        ...
        try {
          const { default: cmd } = await import(fullPath + '?atualizar=' + Date.now()) // fuerza recarga
          if (cmd?.command) {
          ...
          console.error(`Erro ao recarregar comando ${file}:`, err)
        }
      }
    }
  }
  ...
  run: async (client, m) => {
    exec('git pull', async (error, stdout, stderr) => {
      ...
      if (stdout.includes('Already up to date.')) {
        msg = 'ꕥ *Status:* Tudo está atualizado'
      } else {
