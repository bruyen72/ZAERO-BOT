...
export default {
  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    if (!text) return client.reply(m.chat, '《✧》 Forneça um link ou nome do repositório GitHub.', m)
    try {
      await m.react('🕒')
      let info = ''
      let image
      ...
      if (match) {
        ...
        const repoRes = await fetchWithTimeout(`https://api.github.com/repos/${user}/${repo}`)
        const zipRes = await fetchWithTimeout(`https://api.github.com/repos/${user}/${repo}/zipball`)
        ...
        const res = await fetchWithTimeout(`https://api.github.com/search/repositories?q=${encodeURIComponent(text)}`)
        const json = await res.json()
        if (!json.items.length) return client.reply(m.chat, '《✧》 Nenhum resultado encontrado.', m)
        if (json.items.length === 1) {
          const repo = json.items[0]
          const zipRes = await fetchWithTimeout(`https://api.github.com/repos/${repo.owner.login}/${repo.name}/zipball`)
          zipName = zipRes.headers.get('content-disposition')?.match(/filename=(.*)/)?.[1]
          if (!zipName) zipName = `${repo.name}-${repo.owner.login}.zip`
          zipBuffer = Buffer.from(await zipRes.arrayBuffer())
          repos.push(repo)
          image = Buffer.from(await (await fetchWithTimeout(repo.owner.avatar_url)).arrayBuffer())
        } else {
          repos = json.items
          image = Buffer.from(await (await fetchWithTimeout(repos[0].owner.avatar_url)).arrayBuffer())
        }
      }
