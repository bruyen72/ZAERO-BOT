...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    try {
      ...
      if (!mime) return m.reply(`《✧》 Responda a uma *imagem* com:\n${usedPrefix + command}`)
      if (!/^image\/(jpe?g|png|webp)$/i.test(mime)) return m.reply(`《✧》 El formato *${mime || 'desconhecido'}* no es compatible`)

      const buffer = await q.download?.()
      if (!buffer || !Buffer.isBuffer(buffer) || buffer.length < 10) return m.reply('《✧》 Não foi possível baixar a imagem')
      ...
      if (!result?.ok || !result?.buffer) {
        ...
        return m.reply(`《✧》 Não foi possível *melhorar* a imagem (${msg})`)
      }
