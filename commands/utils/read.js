...
export default {
  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    ...
    if (!quoted) return m.reply( '《✧》 Por favor, responda a uma mensagem "ViewOnce" para visualizar seu conteúdo.')
    try {
      await m.react('🕒')
      const content = extractMessageContent(quoted.message || quoted)
      if (!content) return m.reply('《✧》 Não foi possível extrair o conteúdo.')
      ...
      if (!stream) return m.reply('《✧》 Não foi possível baixar o conteúdo.')
