...
export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!user) return m.reply(`「✎」 O usuário mencionado não está cadastrado no bot.`)
    ...
    const txt = `*「✿」Usuário* ◢ ${db.users[who].name} ◤

❖ Nível › *${user.level || 0}*
☆ Experiência › *${user.exp?.toLocaleString() || 0}*
➨ Progresso › *${progresoActual} => ${xp}* _(${porcentaje}%_
✐ Posição › *#${rank}*
❒ Comandos executados › *${user.usedcommands?.toLocaleString() || 0}*`
