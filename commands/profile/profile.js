...
export default {
  ...
  run: async (client, m, args, usedPrefix, command) => {
    ...
    if (!userss) {
      return m.reply('✎ O usuário *mencionado* não está *cadastrado* no bot')
    }
    ...
    const birth = user2.birth || 'Não especificado'
    ...
    const estadoCivil = genero === 'Mujer' ? 'Casado com' : genero === 'Hombre' ? 'Casado com' : 'Casado com'
    const desc = user2.description ? `\n${user2.description}` : ''
    const pasatiempo = user2.pasatiempo ? `${user2.pasatiempo}` : 'Não definido'
...
✿ Nível › *${nivel}*
❀ Experiência › *${exp.toLocaleString()}*
➨ Progresso › *${progreso} => ${xp}* _(${porcentaje}%_
☆ Posição › *#${rank}*

ꕥ Harem › *${haremCount}*
♤ Valor total › *${haremValue.toLocaleString()}*${favLine}
⛁ Coins totales › *¥${totalCoins.toLocaleString()} ${currency}*
❒ Comandos executados › *${comandos.toLocaleString()}*`
