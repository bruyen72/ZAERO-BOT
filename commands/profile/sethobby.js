export default {
  ...
  run: async (client, m, args, usedPrefix) => {
    ...
    if (!input) {
      let lista = '🎯 *Escolha um hobby:*\n\n'
      pasatiemposDisponibles.forEach((pasatiempo, index) => {
        lista += `${index + 1}) ${pasatiempo}\n`
      })
      lista += `\n*Ejemplos:*\n${usedPrefix}sethobby 1\n${usedPrefix}setpastime Ler\n${usedPrefix}setpastime "Outro 🌟"`
    ...
    if (/^\d+$/.test(input)) {
      ...
      if (encontrado) {
        ...
        return m.reply('《✧》 Hobby não encontrado. Use o comando sem argumentos para ver a lista disponível.')
      }
    }
    if (user.pasatiempo === pasatiempoSeleccionado) {
      return m.reply(`《✧》 Você já tem esse hobby estabelecido: *${user.pasatiempo}*`)
    }
    user.pasatiempo = pasatiempoSeleccionado    
    return m.reply(`✐ Seu hobby foi estabelecido:\n> *${user.pasatiempo}*`)
  },
};