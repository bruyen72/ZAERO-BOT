    ...
    for (const userId of botIds) {
      ...
      if (reservedMainNumbers.has(normalizedUserId)) {
        try {
          ...
          console.log(chalk.gray(`[ ✿  ]  Sessão de SUB-BOT inválida removida: ${normalizedUserId}`))
        } catch {
          console.log(chalk.gray(`[ ✿  ]  Não foi possível remover a sessão inválida ${normalizedUserId}`))
        }
        continue
      }
...
if (methodCodeQR) {
  ...
  if (!isInteractive) {
    ...
    opcion = readlineSync.question(chalk.bold.white("\nSelecione uma opção:\n") + chalk.blueBright("1. Com código QR\n") + chalk.cyan("2. Com código de texto de 8 dígitos\n--> "));
    while (!/^[1-2]$/.test(opcion)) {
      console.log(chalk.bold.redBright(`Não são permitidos números que não sejam 1 ou 2, nem letras ou símbolos especiais.`));
      opcion = readlineSync.question("--> ");
    }
    if (opcion === "2") {
      console.log(chalk.bold.redBright(`\nPor favor, insira o número do WhatsApp.\n${chalk.bold.yellowBright("Exemplo: +55119XXXXXXXX")}\n${chalk.bold.magentaBright('---> ')} `));
...
async function startBot() {
  ...
  setTimeout(async () => {
    try {
       if (!state.creds.registered) {
        ...
        console.log(chalk.bold.white(chalk.bgMagenta(`Código de pareamento:`)), chalk.bold.white(chalk.white(codeBot)))
      }
    } catch (err) {
      console.log(chalk.red("Erro ao gerar código:"), err)
    }
  }, 3000)
}
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (opcion == '1' || methodCodeQR) {
      ...
      currentQR = qr
      console.log(chalk.green.bold("[ ✿ ] Escaneie este código QR"));
...
async function init() {
  global.loadDatabase()
  console.log(chalk.gray('[ ✿  ]  Banco de dados carregado com sucesso.'))

  // LOGS DE PERFORMANCE E ESTABILIDADE (2026)
  const isFly = !!process.env.FLY_APP_NAME || !!process.env.FLY_ALLOC_ID;
  const concurrency = globalJobQueue?.concurrency || 1;
  
  console.log(chalk.cyan(`[ SYSTEM ] Modo ${isFly ? 'Fly.io' : 'Local'} detectado.`));
  console.log(chalk.cyan(`[ SYSTEM ] Fila FFmpeg: concorrência = ${concurrency}`));
  if (isFly) console.log(chalk.yellow(`[ SYSTEM ] Otimização: shared-cpu-1x (concorrência limitada).`));
